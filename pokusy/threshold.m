...
% and max thresholds will be active.  if both are set to 0 there is an
% error.
%>> binsToSuppress = [71 72]
%>> threshold2Use = [1 0]
...
% Last Modified by GUIDE v2.5 20-Oct-2011 12:42:58
...
% It will run threshold_OutputFcn and then exit.
...
try
	...
	if (max_value <= 255) && (max_value >= g_MinThreshold) && isnan(max_value) == false
		g_MaxThreshold = round(max_value);
...
try
	...
	if (min_value >= 0) && (min_value <= g_MaxThreshold) && isnan(min_value) == false
		g_MinThreshold = round(min_value);
...
try
	...
	if length(imageDimensions) == 3
		...
		if g_Floating
			...
			minValueS = min(min(imageArray));
			maxValueS = max(max(imageArray));
			rangeS = maxValueS - minValueS;
			normalizedImage = (imageArray - minValueS) / rangeS;
			...
			GLs = rangeS * grayLevelsD + minValueS;
		else
			maxValueForThisDataType = intmax(strDataType);
			...
			[COUNTS, GLs] = imhist(imageArray, numberOfBins);    % make sure you label the axes after imhist because imhist will destroy them.
		end
